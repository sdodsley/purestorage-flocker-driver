...
#    under the License.
"""Pure Storage Block Device Driver."""
...
class PureStorageBlockDriverLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        eliot.Message.new(
            message_type="flocker:node:agents:blockdevice:purestorage",
...
def create_driver_instance(cluster_id, **config):
    ...
    :return: A new PureStorageBlockDeviceAPI object.
    """
    # Configure log routing to the Flocker Eliot logging
    root_logger = logging.getLogger()
    root_logger.addHandler(PureStorageBlockDriverLogHandler())
    root_logger.setLevel(logging.DEBUG)

    config['cluster_id'] = cluster_id
    return PureStorageBlockDeviceAPI(**config)
...
class PureStorageBlockDeviceAPI(object):
    """Block device driver for Pure Storage FlashArray.

    Implements the ``IBlockDeviceAPI`` for interacting with Pure Storage
    array storage.
    """
    ...
    def __init__(self, **kwargs):
        ...
        self._client = purestorage_api.FlashArrayApiHelper(
            kwargs)
    ...
    def allocation_unit(self):
        ...
        The FlashArray recommended minimum is 1 GiB.
    ...
    def compute_instance_id(self):
        ...
        For the FlashArray we use the node's hostname as the identifier.
    ...
    def create_volume_with_profile(self, dataset_id, size, profile_name):
        ...
        with self._client.open_connection() as api:
            try:
                volume = api.create_volume(volume_name,
                                             volume_size,
                                             profile_name)
            except Exception:
                LOG.exception('Error creating volume.')
                raise
        return self._to_blockdevicevolume(volume)
